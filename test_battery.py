...
batts = np.linspace(20,100,21,dtype=int)
#batts = np.linspace(100,1000,46,dtype=int)
print(batts)

csv_res = []

header = ['Total Battery Capacity'] + header

for batt in batts:
    tb = batt
    batteries = [int(batt/4)*3, int(batt/4)]
    print(batteries)
    res = multiple_bs(tc,panels,tb,batteries,bs_number,ks,lams,wr,panel_map,pb_dist,bs_map,bb_dist,beta1,beta2)
    csv_res.append([batt]+res)

with open("Test_Case2_battery.csv", "w", newline="") as f:
    writer = csv.writer(f)
    # Optional header
